...
public class MainActivity11 extends AppCompatActivity implements View.OnClickListener {
            ...
            public void onInit(int status) {
                if (status == TextToSpeech.SUCCESS) {
                    ...
                    if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
                        Toast.makeText(MainActivity11.this, "Deutsch wird nicht unterstützt.", Toast.LENGTH_SHORT).show();
                    } else {
                        // Set the speech rate to a slower value
                        textToSpeech.setSpeechRate(0.7f);
                    }
                } else {
                    Toast.makeText(MainActivity11.this, "TextToSpeech-Initialisierung fehlgeschlagen.", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.audioButton2) {
            String text = "Atmen Sie tief durch die Nase ein und füllen Sie Ihre Lungen mit Luft.";
            speakText(text);
